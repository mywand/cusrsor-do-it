---
description: 代码分析专业场景规范（代码解构、业务分析、系统梳理）
globs: []
alwaysApply: false
---
最近更新: 2025-10-11
...
**场景1：分页逻辑判断错误**
```
❌ 错误判断：看到 skip = pageIndex * pageSize，认为应该是 (pageIndex - 1) * pageSize
✅ 正确做法：
  1. 查看前端代码确认 pageIndex 的起始值
  2. 发现小程序中 pageIndex 从 0 开始
  3. 验证公式：pageIndex=0 → skip=0 ✅ 正确
  4. 理解不同客户端可能有不同的分页约定
```

**场景2：字段取值范围误判**
```
❌ 错误判断：认为 sort 字段不能为负数，需要添加边界检查
✅ 正确做法：
  1. 分析数据类型（Integer 可以存储负数）
  2. 理解业务逻辑（置顶操作：sort = minSort - 1）
  3. 验证排序规则（ORDER BY sort ASC 对负数有效）
  4. 结论：负数是正常的业务逻辑，不需要限制
```

**场景3：枚举值映射误判**
```
❌ 错误判断：仅看后端代码，认为映射关系错误
✅ 正确做法：
  1. 查看前端实际使用代码
  2. 验证前端的跳转逻辑
  3. 确认枚举值的实际含义
...
**典型质疑示例**：
- "为什么这个字段不能为负数？" → 重新审视数据类型和业务逻辑
- "pageIndex 是从 0 开始的" → 重新验证前端实际传值
- "看前端代码，映射关系是反的" → 重新确认前后端约定

#### 6.5 不要过度修复

**警惕"过度保护"**：
- ❌ 不要在没有充分验证的情况下添加"保护性"代码
- ❌ 不要假设所有接口必须使用相同的规则
- ❌ 不要因为"看起来不对"就修改代码
- ✅ 如果代码能正常工作，可能它就是对的
- ✅ 理解业务上下文比修改代码更重要
...
**修复原则**：
1. **确认问题真实存在**：有明确的bug复现或业务影响
2. **理解根本原因**：为什么会出现这个问题
3. **评估修复风险**：修复是否会引入新问题
4. **制定回滚方案**：如果修复有问题如何快速恢复
