...
- **开发阶段修改原则**：数据库开发时，直接修改原始脚本，除非明确说明，否则不创建修复脚本。如 C# 的 Entity Framework，直接修改初始化代码，不创建迁移代码

### 10.4 开发阶段 vs 生产阶段

**开发阶段**（数据库结构频繁变更）：
- ✅ **直接修改原始脚本**：修改初始化脚本或种子数据
- ✅ **重建数据库**：删除数据库重新创建
- ❌ **不创建迁移脚本**：避免迁移脚本堆积

```csharp
// ❌ 开发阶段不推荐：每次修改都创建迁移
Add-Migration AddUserStatus
Add-Migration FixUserStatusType
Add-Migration RenameUserStatusColumn
// 结果：迁移脚本过多，难以维护
...
// 如需修改，创建新的迁移
Add-Migration UpdateUserStatusDefault
Update-Database
```
